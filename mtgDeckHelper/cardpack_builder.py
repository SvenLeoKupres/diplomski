...
def real_main(start_root, num_players_spinbox, rounds_spinbox, mode_spinbox):
    ...
    if mode=="game":
        ...
        shuffled_packs = random.sample(shuffled_packs, 15*num_players*num_rounds)
        save(shuffled_packs, num_players, num_rounds, 15)
        root = PreconstructedWindow(shuffled_packs, num_players, card_pool, removed_cards, assessor)
    elif mode=="load file":
        num_players, num_rounds, num_cards_per_pack, packs = load()
        root = PreconstructedWindow(packs, num_players, card_pool, removed_cards, assessor, num_rounds)
    else:
...
if __name__ == "__main__":
    ...
    title = tk.Label(root, text='(if mode is \"load file\", first 2 values are ignored)')
    title.pack()

    button = ttk.Button(root, text="Cube", command=lambda: real_main(root, player_spinbox, rounds_spinbox, mode_spinbox))
    button.pack()
